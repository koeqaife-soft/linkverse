	...
	async fetch(request: Request) {
		...
		if (!isPublic || request.method != 'GET') {
			try {
				...
				const secretKey = await crypto.subtle.importKey('raw', new TextEncoder().encode(secret), { name: 'HMAC', hash: 'SHA-256' }, false, [
					'verify',
				]);
				...
				const operation = `${request.method}:${key}` as Operation;
